...
### Tests & Tasks
- [x] [M13-T01] Create WalkthroughScreen component
      - Create src/components/setup/WalkthroughScreen.tsx
      - Reusable component for information screens
      - Props: title, icon, description, examples, currentStep, totalSteps, footer
      - Displays formatted content with visual separators

- [x] [M13-T02] Create main setup wizard with step state machine
      - Create src/commands/setup.tsx
      - Define SetupStep union type with all wizard steps
      - Implement state management for each configuration option
      - Handle API key validation and storage
      - Team and initiative selection with Linear API integration

- [x] [M13-T03] Implement existing config detection
      - Check for global config (~/.config/linear-create/)
      - Check for project config (.linear-create/)
      - Display existing configuration details
      - Prompt user to press Enter to exit

- [x] [M13-T04] Implement API key configuration flow
      - Show API key guide screen with instructions
      - Offer choice: config file vs environment variable
      - If env var chosen: validate LINEAR_API_KEY
      - If config file chosen: check if env var exists
      - If env var exists: offer to use env var value or enter manually
      - Validate API key format and connection

- [x] [M13-T05] Implement team selection
      - Fetch teams from Linear API
      - Display interactive team selector
      - Save selected team as defaultTeam
      - Handle errors gracefully

- [x] [M13-T06] Implement initiative selection
      - Fetch initiatives from Linear API
      - Display interactive initiative selector
      - Allow skipping initiative selection
      - Save selected initiative as defaultInitiative

- [x] [M13-T07] Implement optional alias creation
      - Custom multi-select component with arrow key navigation
      - Options: workflow states, project statuses, members
      - Call respective sync-aliases functions
      - Show progress and success messages

- [x] [M13-T08] Create 7 walkthrough information screens
      - Walkthrough 1: Project Creation
      - Walkthrough 2: Alias System
      - Walkthrough 3: Templates
      - Walkthrough 4: Labels & Metadata
      - Walkthrough 5: Configuration Management
      - Walkthrough 6: Common Workflows
      - Walkthrough 7: Help & Resources
      - Each with examples and navigation

- [x] [M13-T09] Remove all automatic progressions
      - Eliminate all setTimeout calls
      - Require manual Enter keypress for all transitions
      - Add proper exit handlers for completion and error screens
      - Create error screens: env-key-missing, env-key-invalid, validation-failed
      - Update all screen footers to show "[Press Enter to continue]"

- [x] [M13-T10] Add config source choice screen
      - New step: config-source-choice
      - Shown when user chooses config file but env var exists
      - Options: Use LINEAR_API_KEY value or Enter manually
      - If using env var: validate and save to config file
      - If manual: proceed to api-key-entry screen

- [x] [M13-T11] Register setup command in CLI
      - Add setup command to src/cli.ts
      - Add comprehensive help text
      - Position as recommended first-run command

- [x] [M13-TS01] Test complete setup flow
      - Build succeeds with all setup code
      - All wizard steps navigate correctly
      - API key validation works
      - Team and initiative selection work
      - Aliases are created successfully
      - No automatic progressions occur

- [x] [M13-TS02] Test smart resume scenarios
      - Existing config detected: shows message, waits for exit
      - Env var detected: shows choice screen
      - Config file chosen with env var: shows source choice
      - Manual entry: blank input screen

- [x] [M13-TS03] Test error handling
      - Invalid API key: shows error screen, waits for exit
      - Missing env var: shows helpful error, waits for exit
      - Network errors: shows validation failed screen
      - All error screens require manual exit
