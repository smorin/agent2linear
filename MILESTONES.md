...
- Glob syntax, `when` structure, the value whitelist, and label uniqueness are validated **before** writing. Single-item `--json` (get/add/edit/remove/move) is a bare object; `list` and `explain`'s `rules[]` are the only arrays. `--dry-run` on `add`/`edit`.
