...
describe('comment add runner', () => {
  it.each([
    ...
    'CMT-TST-%s-ADD validates then guards immediately before one mutation',
    async (kind, target) => {
  ...
  it.each([
    ...
    'CMT-SAF-%s-DRYRUN validates a reply and emits a mutation-free JSON plan',
    async (kind, target) => {
      const h = harness(target);
      vi.mocked(h.dependencies.resolveTarget).mockImplementation(async () => {
      ...
      expect(JSON.parse(h.stdout.join(''))).toEqual({
        ...
        validation: { localWrites: false, targetResolved: true, serverMutation: false },
      });
    }
  );
  ...
  it('CMT-SAF-NO-BLIND-RETRY/CMT-OUT-NO-PARTIAL emits nothing when mutation fails', async () => {
    ...
    await expect(
      runCommentAdd('issue', 'ENG-123', { body: 'hello' }, h.dependencies)
...
describe('comment list runner', () => {
  it.each([
    ...
    'CMT-TST-%s-LIST passes raw pagination and never invokes the mutation guard',
    async (kind, target) => {
      const h = harness(target);
      ...
      expect(h.dependencies.recordHistory).toHaveBeenCalledWith(
  ...
  it('CMT-OPT-IL-LIMIT/AFTER rejects invalid pagination before target or API calls', async () => {
    for (const options of [{ limit: '0' }, { limit: '251' }, { limit: '1.5' }, { after: '' }]) {
      const h = harness();
      await expect(
        runCommentList('issue', 'ENG-123', options, h.dependencies)
  ...
  it('CMT-OUT-NO-PARTIAL emits nothing when a page traversal rejects', async () => {
    ...
    await expect(runCommentList('issue', 'ENG-123', { all: true }, h.dependencies)).rejects.toThrow(
      'page two failed'
    );
    expect(h.stdout).toEqual([]);
  });
